...
//        debugPrint("tapped\(indexPath.row)")
        let url = self.news["stories"].arrayValue[indexPath.row]["id"].stringValue
...
//                debugPrint(self.storytitle.count)
    ...
    func getBefore(_ date:Date){
        let api = "https://news-at.zhihu.com/api/3/stories/before/"+date.year.description+date.month.description+date.day.description
        print(api)
